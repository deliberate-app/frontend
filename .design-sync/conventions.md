...
- `market-facts` — a label/figure `<dl>` for a modal's readout, one fact per row.
- `stake-range` + `stake-submit` — the signed stake slider and its stance-colored confirm.
...
collapsed and open on their own trigger. `StakeModal` likewise owns its slider position, starting at
the neutral centre. Drive them through their own controls — a click, or a native `input` event on
the range — never by faking state.
