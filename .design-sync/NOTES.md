...
- **The inputs belong on `main`.** The first sync committed `.design-sync/` on a branch
  (`design-sync-import`, never merged); by the next sync `main` was 18 commits ahead and the config,
  conventions and previews described components that had since changed - the re-sync could not run
  until they were restored. They live on `main` now. Keep them there: they only stay true if they
  travel with the components they describe.
- **Driving a control the preview cannot reach through props.** `StakeModal`'s slider position is
  internal state, so its preview sets the range through the native value setter and dispatches an
  `input` event (see `previews/StakeModal.tsx`) - React ignores a plain `.value =`. Same principle as
  the click-your-own-trigger rule below: drive the real control, never fake the state.
