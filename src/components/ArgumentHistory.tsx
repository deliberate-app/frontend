...
export function ArgumentHistory({
  ...
  if (points.length < 2 || totalDebateStake <= 0) {
    return null;
  }
  ...
  const x = (at: number) => padLeft + ((at - from) / span) * plotWidth;
  ...
  const path = (coords: ReadonlyArray<[number, number]>) =>
  ...
  const area = (coords: ReadonlyArray<[number, number]>, baseline: number) =>
  ...
  return (
    <figure className="history">
      ...
      >
        ...
        <path className="history-band history-subtree" d={area(stepped((p) => p.subtreeStake, yStake), floor)} />
        <path className="history-band history-stake" d={area(stepped((p) => p.stake, yStake), floor)} />
        ...
        <path className="history-line history-market" d={path(stepped((p) => p.market, yRating))} />
        <path className="history-line history-rating" d={path(stepped((p) => p.rating, yRating))} />
